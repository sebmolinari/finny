...
// Get portfolio holdings for the authenticated user
router.get("/portfolio/holdings", authMiddleware, (req, res) => {
  try {
    /**
     * @swagger
     * /analytics/portfolio/holdings:
     *   get:
     *     summary: Get portfolio holdings for the authenticated user
     *     tags: [Analytics]
     *     security:
     *       - bearerAuth: []
     *     parameters:
     ...
     *         schema:
     *           type: boolean
     *         description: Filter out holdings with zero or near-zero quantity (default true)
     *     responses:
     *       200:
     ...
     *         description: Server error
     */
    const { hideZeroQuantity } = req.query;
    const hideZero = hideZeroQuantity !== "false"; // Default to true unless explicitly set to 'false'
